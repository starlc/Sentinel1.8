...
public class DemoClusterInitFuncByZk implements InitFunc {
    ...
    private void registerClusterRuleSupplier() {
        ...
        ClusterFlowRuleManager.setPropertySupplier(namespace -> {
            ...
            ReadableDataSource<String, List<FlowRule>> ds = new ZookeeperDataSource<>(remoteAddress, PRE_PATH + "/" + namespace + DemoConstants.FLOW_POSTFIX, 
                source -> JSON.parseObject(source, new TypeReference<List<FlowRule>>() {}));
            return ds.getProperty();
        });
        // 注册集群参数流控规则属性提供者，同样根据命名空间创建数据源
        ClusterParamFlowRuleManager.setPropertySupplier(namespace -> {
            // 为每个命名空间创建一个ZooKeeper数据源，从ZooKeeper配置中心读取该命名空间的参数流控规则
            ReadableDataSource<String, List<ParamFlowRule>> ds = new ZookeeperDataSource<>(remoteAddress, PRE_PATH + "/" + namespace + DemoConstants.PARAM_FLOW_POSTFIX, 
    ...
    private void initClientServerAssignProperty() {
        ...
        ReadableDataSource<String, ClusterClientAssignConfig> clientAssignDs = new ZookeeperDataSource<>(remoteAddress, PRE_PATH + "/" + clusterMapDataId, 
            source -> {
            // 解析集群映射数据，获取集群分组列表
            List<ClusterGroupEntity> groupList = JSON.parseObject(source, new TypeReference<List<ClusterGroupEntity>>() {});
    ...
    private void initStateProperty() {
        ...
        // [{"clientSet":["112.12.88.66@8729","112.12.88.67@8727"],"ip":"112.12.88.68","machineId":"112.12.88.68@8728","port":11111}]
        // machineId: <ip@commandPort>, commandPort for port exposed to Sentinel dashboard (transport module)
        ReadableDataSource<String, Integer> clusterModeDs = new ZookeeperDataSource<>(remoteAddress, PRE_PATH + "/" + clusterMapDataId, 
            source -> {
            // 解析集群映射数据，获取集群分组列表
            List<ClusterGroupEntity> groupList = JSON.parseObject(source, new TypeReference<List<ClusterGroupEntity>>() {});
            // 从集群分组列表中提取当前机器的集群模式
