...
/**
 * 基于ZooKeeper实现的只读数据源，用于从ZooKeeper中读取Sentinel规则配置。
 * 该类通过ZooKeeper的节点监听机制，实现了配置的动态推送和更新。
 * 当ZooKeeper中的配置发生变化时，会自动触发监听器更新Sentinel中的规则。
 *
 * @author guonanjun
 */
public class ZookeeperDataSource<T> extends AbstractDataSource<String, T> {
     ...
     * @param path 配置存储的节点路径
     * @param parser 将ZooKeeper中存储的字符串转换为目标规则对象的转换器
     */
